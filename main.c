...
{	
	...
	int skype_status, flash_status, codecs_status;
	...
	if(getuid()!=0)
	{
		...
		GtkWidget *janela, *label, *skype_button_label;
		GtkWidget *choice_box;
		GtkWidget *extras0, *skype_button;
		GtkWidget *extras1, *flash_button;
		GtkWidget *extras2, *codecs_button;
		GtkWidget *extras3, *quit_button, *valign;
		...
		choice_box = gtk_vbox_new(TRUE, 6);
		extras0 = gtk_hbox_new(TRUE, 6);
		extras1 = gtk_hbox_new(TRUE, 6);
		extras2 = gtk_hbox_new(TRUE, 6);
		extras3 = gtk_hbox_new(TRUE, 6);
		label = gtk_label_new("Select an option or press Quit");
		skype_button_label =  gtk_label_new("mimimimimimim");
		valign = gtk_alignment_new(6, 6, 6, 6);
		...
		gtk_container_add(GTK_CONTAINER(extras3), valign);
		...
		gtk_box_pack_start (GTK_BOX (extras0), skype_button, 6, 6, 6);
		////////////////
		...
		gtk_box_pack_start (GTK_BOX (extras1), flash_button, 6, 6, 6);
		////////////////

		if(TRUE){
			codecs_button = gtk_button_new_with_label("uninstall codecs!");
			codecs_status = 0;
		}else{
			codecs_button = gtk_button_new_with_label("uninstall codecs!");
			codecs_status = 1;
		}
		gtk_box_pack_start (GTK_BOX (extras2), codecs_button, 6, 6, 6);

		quit_button = gtk_button_new_with_label("Close");
		gtk_box_pack_start (GTK_BOX (extras3), quit_button, 6, 6, 6);
		...
		if(codecs_status == 1){
			(g_signal_connect(flash_button, "clicked",
					  G_CALLBACK(install_flash), NULL));
				gtk_label_set_text(GTK_LABEL(codecs_button), "Uninstall Codecs");
		}else{
			(g_signal_connect(flash_button, "clicked",
					  G_CALLBACK(uninstall_flash), NULL));
				gtk_label_set_text(GTK_LABEL(codecs_button), "Uninstall Codecs");
